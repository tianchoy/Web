(function($){
	var bigAutocomplete = new function(){
		...
		this.init = function(abc){
			$(".search_div" + abc).append("<div class='bigAutocompleteContent"+abc+" bigautocomplete-layout'></div>");
			$(document).bind('mousedown',function(event){
				var $target = $(event.target);
				if((!($target.parents().andSelf().is('.bigAutocompleteContent'+abc))) && (!$target.is($(currentInputText)))){
					bigAutocomplete.hideAutocomplete(abc);
				}
			})
			
			//鼠标悬停时选中当前行
			$(".bigAutocompleteContent"+abc).delegate("tr", "mouseover", function() {
				$(".bigAutocompleteContent"+abc+" tr").removeClass("ct");
				$(this).addClass("ct");
			...
			$(".bigAutocompleteContent"+abc).delegate("tr", "click", function() {
				$(currentInputText).val( $(this).find("div:last").html());
				var callback_ = $(currentInputText).data("config").callback;
				if($(".bigAutocompleteContent"+abc).css("display") != "none" && callback_ && $.isFunction(callback_)){
		...
		this.autocomplete = function(param){
			if($(".search_div" + param['abc']).length > 0){
				bigAutocomplete.init(param['abc']);//初始化信息
			}			
			var $this = this;//为绑定自动补全功能的输入框jquery对象
			
			var $bigAutocompleteContent = $(".bigAutocompleteContent"+ param['abc']);
			...
			$this.keydown(function(event) {
				...
				case 27://ESC键隐藏下拉框
					...
					bigAutocomplete.hideAutocomplete(param['abc']);
					break;
				}
			});		
		...
		//隐藏下拉框
		this.hideAutocomplete = function(abc){
			var $bigAutocompleteContent = $(".bigAutocompleteContent"+abc);
			if($bigAutocompleteContent.css("display") != "none"){
				$bigAutocompleteContent.find("tr").removeClass("ct");
				$bigAutocompleteContent.hide();
			}			
